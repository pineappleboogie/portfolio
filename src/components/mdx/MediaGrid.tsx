...
/**
 * MediaGrid component for displaying multiple images/videos in a two-column grid
 *
 * Features:
 * - Two-column grid layout on desktop
 * - Stacks vertically on mobile
 * - Auto-play videos when they scroll into view
 * - Pause/play button for each video (bottom-right corner)
 * - Always-visible captions, center-aligned below each item
 */
export function MediaGrid({ items }: MediaGridProps) {
  return (
    <div className="mb-10 grid grid-cols-1 md:grid-cols-2 gap-6">
      {items.map((item, index) => (
        <MediaGridItem key={index} {...item} />
      ))}
    </div>
  );
}
...
/**
 * Individual media item within the grid
 * Handles video auto-play with Intersection Observer
 */
function MediaGridItem({ src, alt, type, caption, poster }: MediaItem) {
